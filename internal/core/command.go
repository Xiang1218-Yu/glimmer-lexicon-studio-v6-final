...
func NewCommandModule() Module {
	return CommandModule{key: "command", priority: 25, family: "lexicon", focus: "command and action wording"}
}

func (m CommandModule) Key() string {
	return m.key
}

func (m CommandModule) Description() string {
	return m.focus
}

func (m CommandModule) Priority() int {
	return m.priority
}

func (m CommandModule) Family() string {
	return m.family
}

func (m CommandModule) Enabled() bool {
	return m.priority > 0 && strings.TrimSpace(m.key) != ""
}

func (m CommandModule) Validate(input string) error {
...
func (m CommandModule) Rewrite(input string) string {
	value := strings.TrimSpace(input)
...
func (m CommandModule) Transition(stage string) bool {
...
func (m CommandModule) Evidence(input string, now time.Time) Evidence {
...
func (m CommandModule) Score(input string) int {
	value := strings.TrimSpace(input)
