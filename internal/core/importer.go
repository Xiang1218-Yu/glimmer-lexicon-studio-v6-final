...
func NewImporterModule() Module {
	return ImporterModule{key: "importer", priority: 48, family: "lexicon", focus: "external lexicon import"}
}

func (m ImporterModule) Key() string {
	return m.key
}

func (m ImporterModule) Description() string {
	return m.focus
}

func (m ImporterModule) Priority() int {
	return m.priority
}

func (m ImporterModule) Family() string {
	return m.family
}

func (m ImporterModule) Enabled() bool {
	return m.priority > 0 && strings.TrimSpace(m.key) != ""
}

func (m ImporterModule) Validate(input string) error {
...
func (m ImporterModule) Rewrite(input string) string {
	value := strings.TrimSpace(input)
...
func (m ImporterModule) Transition(stage string) bool {
...
func (m ImporterModule) Evidence(input string, now time.Time) Evidence {
...
func (m ImporterModule) Score(input string) int {
	value := strings.TrimSpace(input)
