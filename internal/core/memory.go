...
func NewMemoryModule() Module {
	return MemoryModule{key: "memory", priority: 50, family: "lexicon", focus: "translation memory links"}
}

func (m MemoryModule) Key() string {
	return m.key
}
...
func (m MemoryModule) Priority() int {
	return m.priority
}

func (m MemoryModule) Family() string {
	return m.family
}

func (m MemoryModule) Enabled() bool {
	return m.priority > 0 && strings.TrimSpace(m.key) != ""
}

func (m MemoryModule) Validate(input string) error {
...
func (m MemoryModule) Rewrite(input string) string {
	value := strings.TrimSpace(input)
...
func (m MemoryModule) Transition(stage string) bool {
...
func (m MemoryModule) Evidence(input string, now time.Time) Evidence {
...
func (m MemoryModule) Score(input string) int {
	value := strings.TrimSpace(input)
