...
func NewChecksumModule() Module {
	return ChecksumModule{key: "checksum", priority: 47, family: "lexicon", focus: "bundle checksum calculation"}
}

func (m ChecksumModule) Key() string {
	return m.key
}

func (m ChecksumModule) Description() string {
	return m.focus
}

func (m ChecksumModule) Priority() int {
	return m.priority
}

func (m ChecksumModule) Family() string {
	return m.family
}

func (m ChecksumModule) Enabled() bool {
	return m.priority > 0 && strings.TrimSpace(m.key) != ""
}

func (m ChecksumModule) Validate(input string) error {
...
func (m ChecksumModule) Rewrite(input string) string {
	value := strings.TrimSpace(input)
...
func (m ChecksumModule) Transition(stage string) bool {
...
func (m ChecksumModule) Evidence(input string, now time.Time) Evidence {
...
func (m ChecksumModule) Score(input string) int {
	value := strings.TrimSpace(input)
