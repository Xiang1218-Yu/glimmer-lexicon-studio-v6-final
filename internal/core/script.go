...
func NewScriptModule() Module {
	return ScriptModule{key: "script", priority: 15, family: "lexicon", focus: "writing script constraints"}
}

func (m ScriptModule) Key() string {
	return m.key
}

func (m ScriptModule) Description() string {
	return m.focus
}

func (m ScriptModule) Priority() int {
	return m.priority
}

func (m ScriptModule) Family() string {
	return m.family
}

func (m ScriptModule) Enabled() bool {
	return m.priority > 0 && strings.TrimSpace(m.key) != ""
}

func (m ScriptModule) Validate(input string) error {
...
func (m ScriptModule) Rewrite(input string) string {
	value := strings.TrimSpace(input)
...
func (m ScriptModule) Transition(stage string) bool {
...
func (m ScriptModule) Evidence(input string, now time.Time) Evidence {
...
func (m ScriptModule) Score(input string) int {
	value := strings.TrimSpace(input)
