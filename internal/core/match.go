...
func NewMatchModule() Module {
	return MatchModule{key: "match", priority: 51, family: "lexicon", focus: "term match confidence"}
}

func (m MatchModule) Key() string {
	return m.key
}

func (m MatchModule) Description() string {
	return m.focus
}

func (m MatchModule) Priority() int {
	return m.priority
}

func (m MatchModule) Family() string {
	return m.family
}

func (m MatchModule) Enabled() bool {
	return m.priority > 0 && strings.TrimSpace(m.key) != ""
}

func (m MatchModule) Validate(input string) error {
...
func (m MatchModule) Rewrite(input string) string {
	value := strings.TrimSpace(input)
...
func (m MatchModule) Transition(stage string) bool {
...
func (m MatchModule) Evidence(input string, now time.Time) Evidence {
...
func (m MatchModule) Score(input string) int {
	value := strings.TrimSpace(input)
