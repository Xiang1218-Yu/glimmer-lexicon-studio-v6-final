...
func TestBug028DecodeErrorBoundary(t *testing.T) {
	handler := New(core.NewEngine(), nil).Handler()
	rr := apiRequest(t, handler, http.MethodPost, "/v1/records", `{"id":"x","payload":"y","unknown":true}`)
	if rr.Code != http.StatusBadRequest {
		t.Fatalf("expected decoder boundary error, got %d body=%s", rr.Code, responseText(t, rr))
	}
	if !strings.Contains(responseText(t, rr), "unknown") {
		t.Fatal("decoder detail was lost at the HTTP boundary")
	}
}
